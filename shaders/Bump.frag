...
void main() {
  ...
  vec3 bumped_normal = normalize(TBN * local_normal);
  ...
  float diffuse_term = max(0.0, dot(bumped_normal, l));
  float specular_term = pow(max(0.0, dot(bumped_normal, half_vec)), shininess);
